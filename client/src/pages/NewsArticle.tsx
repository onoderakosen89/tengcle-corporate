  ...
  },
  {
    ...
    content: {
      en: [
        ...
        "Our hotel operations services draw on experience developed through Tengcle-related work, including hospitality activities in Japan.",
...
export default function NewsArticle() {
  ...
  const fontClass =
    pathLang === "ja" ? "font-jp" : pathLang === "zh" ? "font-zh" : "";
  ...
  const text =
    translations[pathLang as keyof typeof translations] || translations.en;
  ...
  const title =
    article.title[pathLang as keyof typeof article.title] || article.title.en;
  const excerpt =
    article.excerpt[pathLang as keyof typeof article.excerpt] ||
    article.excerpt.en;
  const content =
    article.content[pathLang as keyof typeof article.content] ||
    article.content.en;
  const category =
    article.category[pathLang as keyof typeof article.category] ||
    article.category.en;
  const readTime =
    article.readTime[pathLang as keyof typeof article.readTime] ||
    article.readTime.en;
    ...
    <>
      ...
      <div className="min-h-screen bg-cream">
        ...
        <section
          ...
          style={{
            backgroundImage: `linear-gradient(rgba(88, 28, 135, 0.85), rgba(88, 28, 135, 0.9)), url(${article.image})`,
          }}
        >
          <div className="container relative z-10">
            ...
            >
              ...
              <h1
                className={`text-3xl md:text-4xl lg:text-5xl font-heading text-white max-w-4xl ${fontClass}`}
              >
                {title}
              </h1>
            </motion.div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              >
                {content.map((paragraph, index) => (
                  <p
                    key={index}
                    className={`text-charcoal/80 leading-relaxed mb-6 last:mb-0 ${fontClass}`}
                  >
                    {paragraph}
                  </p>
                ))}
        ...
        {relatedArticles.length > 0 && (
          <section className="py-16 bg-white">
            <div className="container">
              <h2
                className={`text-2xl font-heading text-charcoal mb-8 ${fontClass}`}
              >
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {relatedArticles.map(related => {
                  const relatedTitle =
                    related.title[pathLang as keyof typeof related.title] ||
                    related.title.en;
                  const relatedExcerpt =
                    related.excerpt[pathLang as keyof typeof related.excerpt] ||
                    related.excerpt.en;

                  return (
                    <Link
                      key={related.id}
                      href={`${basePath}/news/${related.slug}`}
                    >
                      <motion.article
                        whileHover={{ y: -5 }}
                      ...
                      >
                        ...
                        <h3
                          className={`text-lg font-heading text-charcoal mb-2 group-hover:text-purple transition-colors ${fontClass}`}
                        >
                          {relatedTitle}
                        </h3>
                        <p
                          className={`text-slate text-sm line-clamp-2 ${fontClass}`}
                        >
                          {relatedExcerpt}
                        </p>
                      </motion.article>
